...
plt1.plot(samples[:, :, 0][0:200],"-k",linewidth="0.5") # a values
plt1.set_xlabel("step number")
plt1.set_ylabel("a")


plt2=fig.add_subplot(spec[1,:])
plt2.plot(samples[:, :, 1][0:200],'-k',linewidth='0.5') # b values
plt2.set_xlabel("step number")
plt2.set_ylabel("b")


plt3=fig.add_subplot(spec[2,:])
plt3.plot(samples[:, :, 2][0:200],'-k',linewidth='0.5') # c values
...
for i in range(200):
    ...
    b=data[r,1]
    c=data[r,2]
    plt.plot(X,a*X**2+b*X+c,'y')

x=np.sort(x)    
plt.plot(X,a_true*X**2+b_true*X+c_true,label="best fit")
plt.errorbar(x,y,yerr=yerr,fmt="ro",label="data with errorbars")
plt.xlabel("x")
plt.ylabel("y")
